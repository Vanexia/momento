...
def _persist_setup_config(config: Config) -> OSError | None:
    """Persist setup before applying its optional Windows startup entry."""
    save_config(config)
    try:
...
class WelcomeDialog(QDialog):
    ...
    def _on_finish(self) -> None:
        # An empty/whitespace recordings-folder field resolves to Path('.') (the
        # process CWD), which would silently misfile every recording there and
        # make the library look empty on the next launch. Unlike SettingsPanel,
        # the wizard builds the Config via dataclasses.replace (bypassing
        # Config.from_dict's emptiness guard), so sanitise here: treat a
        # blank/dot path as "unset" and keep the existing configured folder.
        folder_text = self._folder_edit.text().strip()
        if not folder_text:
            QMessageBox.warning(self, "Momento", "Choose a recordings folder first.")
            return
        try:
            self._pending["output_folder"] = Path(folder_text).expanduser().resolve()
        except (OSError, RuntimeError) as e:
            QMessageBox.warning(self, "Momento", f"That folder is invalid:\n{e}")
            return
        try:
            new_cfg = dataclasses.replace(
                self._config, setup_complete=True, **self._pending
            )
        except Exception:
            ...
            self.reject()
            return
        if not new_cfg.mic_device or not new_cfg.system_audio_device:
            missing = []
            if not new_cfg.mic_device:
                missing.append("microphone")
            if not new_cfg.system_audio_device:
                missing.append("system audio")
            reply = QMessageBox.question(
                self,
                "Continue without audio?",
                "No " + " or ".join(missing) + " is selected. Recordings will "
                "not include that audio. Continue anyway?",
        ...
        output_folder = Path(new_cfg.output_folder).expanduser()
        try:
            output_folder.mkdir(parents=True, exist_ok=True)
            with tempfile.NamedTemporaryFile(dir=output_folder, prefix=".momento-"):
                pass
        except OSError as e:
            logger.exception("Wizard: recordings folder is not writable")
            QMessageBox.warning(
                self, "Momento", f"Momento cannot write to that folder:\n{e}"
            )
            return
        try:
            autostart_error = _persist_setup_config(new_cfg)
        except OSError as e:
            QMessageBox.critical(
                self, "Momento", f"Could not finish setup:\n{e}"
            )
            return
        if autostart_error is not None:
            QMessageBox.warning(
                self,
                "Momento",
                "Setup was saved, but Momento could not update Windows startup:\n"
                f"{autostart_error}",
            )
    ...
    def _build_audio_page(self) -> QWidget:
        ...
        self._pending["mic_device"] = self._wizard_mic_combo.currentData() or ""
        ...
        self._pending["system_audio_device"] = (
            self._wizard_sys_combo.currentData() or ""
        )
        ...
        self._sys_test_btn.setToolTip(
            "Plays a short chime through the selected playback device so you "
        ...
        refresh.clicked.connect(self._reload_audio_devices)
        form.addRow("", refresh)
    ...
    def _reload_audio_devices(self) -> None:
        """Re-enumerate devices and reconcile persistence with visible choices."""
        if self._mic_monitor is not None and self._mic_monitor.is_running:
            self._mic_monitor.stop()
        current_mic = self._wizard_mic_combo.currentData() or ""
        current_sys = self._wizard_sys_combo.currentData() or ""
        self._mic_devices = list_mic_devices()
        self._sys_devices = list_loopback_devices()

        for combo, devices, previous, field in (
            (self._wizard_mic_combo, self._mic_devices, current_mic, "mic_device"),
            (
                self._wizard_sys_combo,
                self._sys_devices,
                current_sys,
                "system_audio_device",
            ),
            ...
            _select_combo_by_value(combo, previous)
            combo.blockSignals(False)
            self._pending[field] = combo.currentData() or ""
        self._refresh_wizard_device_status()
    ...
    def _on_sys_test_clicked(self) -> None:
        ...
        if not device_id:
            QMessageBox.information(
                self, "Momento", "Pick a system audio device first."
            )
            return
        ...
        if target is None:
            QMessageBox.warning(
                self,
                "Momento",
                "That playback device is unavailable. Refresh the devices or "
                "choose another one.",
            )
            return
        player = QMediaPlayer(self)
        out = QAudioOutput(target, self)
        out.setVolume(0.6)
    ...
    def _build_capture_page(self) -> QWidget:
        ...
        for label, value in (
            ...
            ("Custom 16 Mbit/s (recommended)", "custom"),
        ):
            self._quality_combo.addItem(label, value)
        _select_combo_by_value(
            self._quality_combo,
            self._config.quality_preset
            if self._config.quality_preset in {"low", "medium", "high", "custom"}
            else "custom",
        )
        self._quality_combo.currentIndexChanged.connect(self._on_wizard_quality_changed)
    ...
    def _on_wizard_quality_changed(self, _index: int) -> None:
        value = self._quality_combo.currentData() or "custom"
        self._pending["quality_preset"] = value
        if value == "custom":
            self._pending["custom_bitrate_kbps"] = 16_000
