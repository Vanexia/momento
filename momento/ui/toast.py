...
# Visual tokens — sourced from the Obsidian theme so the toast can't drift from
# the app palette. Status accents (warn/bookmark) intentionally keep their
# semantic amber/orange; only the "saved" accent tracks the brand violet.
_BG = QColor(_theme.BG_CARD_RAISED)        # bg/card-raised
_BORDER = QColor(_theme.BORDER)            # border/default
_ACCENT_REC = QColor(_theme.EVENT_KILL)    # red dot — currently recording
_ACCENT_IDLE = QColor(_theme.ACCENT_VIOLET)  # violet — recording saved (brand)
_ACCENT_WARN = QColor("#ecb14c")           # amber — couldn't start, action needed
_ACCENT_BOOKMARK = QColor("#ffaa3c")       # orange — bookmark dropped
_TITLE = QColor(_theme.TEXT_PRIMARY)
_SUBTITLE = QColor(_theme.TEXT_MID)

# Visible body size. WIDTH is fixed; HEIGHT adapts to the (word-wrapped) text so
# a long message grows the toast down instead of being truncated — capped so a
# pathological message can't fill the screen. The window itself is larger than
# the body so the drop shadow has room to spill beyond it without bumping the
# window edge — without this padding Windows logs UpdateLayeredWindowIndirect
# failures because the shadow's dirty rect extends past the destination rect.
_BODY_W = 360
_BODY_H_MIN = 84    # short toasts keep the compact look they had
_BODY_H_MAX = 280   # cap so the toast can't grow unbounded
_SHADOW_PAD = 20    # px on each side
_W = _BODY_W + 2 * _SHADOW_PAD
_ICON_BOX = 56
_PAD = 12
_VPAD = 14          # top/bottom padding around the text block
_TEXT_GAP = 4       # gap between the title and subtitle lines
_RADIUS = 12
_MARGIN_FROM_EDGE = 24   # px from the top-left of the screen

# Left | Top | word-wrap, as an int (mixing AlignmentFlag + TextFlag).
_WRAP_FLAGS = (
...
class RecordingToast(QWidget):
    ...
    def __init__(self) -> None:
        ...
        # Body height is recomputed per-toast from its text (see _relayout);
        # start at the minimum.
        self._body_h = _BODY_H_MIN
        self.setFixedSize(_W, _BODY_H_MIN + 2 * _SHADOW_PAD)
    ...
    def _present(self, duration_ms: int) -> None:
        self._relayout()  # size the window to the current title/subtitle first
        self._reposition()
    ...
    def _reposition(self) -> None:
        ...
        bottom = geo.top() + geo.height() - self.height() - _MARGIN_FROM_EDGE
    ...
    def _text_col_width(self) -> int:
        """Width of the text column, mirroring paintEvent's geometry."""
        text_left = _SHADOW_PAD + _PAD + _ICON_BOX + 14
        text_right = _SHADOW_PAD + _BODY_W - _PAD
        return max(40, text_right - text_left)

    def _wrapped_heights(self) -> tuple[int, int]:
        """(title_height, subtitle_height) when word-wrapped to the text column."""
        text_w = self._text_col_width()
        th = QFontMetrics(self._title_font()).boundingRect(
            0, 0, text_w, 4000, _WRAP_FLAGS, self._title or ""
        ).height()
        sh = 0
        if self._subtitle:
            sh = QFontMetrics(self._subtitle_font()).boundingRect(
                0, 0, text_w, 4000, _WRAP_FLAGS, self._subtitle
            ).height()
        return th, sh

    def _relayout(self) -> None:
        """Recompute the body height from the current text and resize the window
        so a long message wraps and grows down rather than being truncated."""
        th, sh = self._wrapped_heights()
        block = th + (_TEXT_GAP if sh else 0) + sh
        content = max(_ICON_BOX, block)
        self._body_h = max(_BODY_H_MIN, min(_BODY_H_MAX, content + 2 * _VPAD))
        self.setFixedSize(_W, self._body_h + 2 * _SHADOW_PAD)
    ...
    def paintEvent(self, _event) -> None:  # noqa: N802 (Qt API)
        ...
        # Body rect, inset by the shadow padding. Height is dynamic (_relayout).
        body_rect = QRect(_SHADOW_PAD, _SHADOW_PAD, _BODY_W, self._body_h)
        ...
        icon_rect = QRect(
            body_rect.left() + _PAD,
            body_rect.top() + (self._body_h - _ICON_BOX) // 2,
            _ICON_BOX,
            _ICON_BOX,
        )
        ...
        # Text column to the right of the icon — word-wrapped and vertically
        # centred as a block, so the toast grows down for longer messages
        ...
        th, sh = self._wrapped_heights()
        block_h = th + (_TEXT_GAP if sh else 0) + sh
        block_top = body_rect.top() + max(_VPAD, (self._body_h - block_h) // 2)

        painter.setFont(self._title_font())
        painter.setPen(_TITLE)
        painter.drawText(QRect(text_left, block_top, text_w, th), _WRAP_FLAGS, self._title)

        if self._subtitle:
            painter.setFont(self._subtitle_font())
            painter.setPen(_SUBTITLE)
            painter.drawText(
                QRect(text_left, block_top + th + _TEXT_GAP, text_w, sh),
                _WRAP_FLAGS,
                self._subtitle,
            )
