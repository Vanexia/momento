...
class MomentoTray(QObject):
    ...
    _recording_finished_signal = pyqtSignal(str)  # final/recoverable media path
    ...
    def __init__(
        ...
        self._recording_finished_signal.connect(self._on_recording_finished_qt)

    # ------------------------------------------------------------------ API
    def show(self) -> None:
        self._tray.show()
        # Pre-warm the editor (and its settings panel) in the background a
        # moment after the app settles in the tray, so the first click shows
        # an already-built window instantly instead of stalling ~1s while it
        # constructs + pays Qt's one-time style warmup. Safe now that the
        # white-flash on showing a pre-built hidden window is fixed
        # (EditorWindow.showEvent opacity reveal). Skipped when a custom
        # open_editor handler owns the editor lifecycle.
    ...
    def on_recording_finished(self, path) -> None:
        """Session callback for a clean or recoverable recording file."""
        self._recording_finished_signal.emit(str(path))

    def _on_failure(self, reason: str, game: ActiveGame, detail: str) -> None:
        # A dirty finalize means a recording WAS in flight. Cancel its pending
        ...
        try:
            ...
            if reason == FAILURE_FINALIZE:
                # Recorded, but the file didn't close cleanly — not a
                # "couldn't record"; it exists and is recoverable.
                title = f"{display} recording may be incomplete"
            elif reason == FAILURE_AUDIO:
                # Recording IS running, just missing a mic/system-audio leg.
                title = f"{display}: audio not captured"
            elif reason == FAILURE_SOFTWARE_ENCODER:
    ...
    def _apply_status(self, status: str, game: ActiveGame | None) -> None:
        if status == STATUS_RECORDING and game is not None:
            ...
            if self._last_game_name is not None:
                ...
                self._last_game_name = None
                # The finished-file callback handles the live editor update.
    ...
    def _on_recording_finished_qt(self, path: str) -> None:
        """Live-refresh the open editor using the single finished file path."""
        try:
            ed = self._editor
            if ed is not None and ed.isVisible():
                if not ed.add_finished_recording(path, preserve_selection=True):
                    ed.refresh(preserve_selection=True)
        except Exception:
            logger.exception("Live editor finished-recording update failed")
    ...
    def _on_open_editor(self) -> None:
        ...
        try:
            ...
            return
        # Refresh in case new recordings landed since it was opened, but keep
        # the user's last selected clip/position when reopening from the tray.
        self._editor.refresh(preserve_selection=True)

    def _build_editor(self) -> EditorWindow:
        """Construct + wire the editor window without showing it.

        Split out from ``_ensure_editor`` so it can run on a post-startup idle
        timer. The window stays at opacity 0 until shown (EditorWindow), so
        pre-building it hidden no longer flashes on first show.
        """
    ...
    def _prebuild_editor(self) -> None:
        """Warm the editor + settings panel in the background so the first
        click is instant. Best-effort: a failure leaves ``self._editor`` None,
        so ``_ensure_editor`` still builds it lazily on click.
        """
        try:
            editor = self._build_editor()
            # Build the (heavy, ~0.7s cold) settings panel now too, while the
            # window is hidden, so opening Settings later is instant.
            editor.prewarm_settings()
        except Exception:
            logger.exception("Editor prebuild failed (will build lazily on click)")

    def _ensure_editor(self) -> EditorWindow:
        """Build the editor if needed, then show it AND pull it to the front
        with focus. A tray click should pop the window up in front of the
        user, not just flash it on the taskbar."""
        ed = self._build_editor()
        if ed.isMinimized():
            ed.showNormal()
        elif not ed.isVisible():
            ed.show()
        ed.raise_()
        ed.activateWindow()
        # Windows' foreground lock downgrades activateWindow() to a taskbar
        # flash when the triggering click came from the shell-owned tray.
        # Force it to the foreground via the AttachThreadInput trick.
        try:
            from momento.util.windows_api import force_foreground_window
            force_foreground_window(int(ed.winId()))
        except Exception:
            logger.exception("Could not force editor to foreground")
        return ed
    ...
    def _apply_new_config(self, new_cfg: Config) -> None:
        ...
        logger.info(
            "Settings saved: mic=%s system=%s output_configured=%s hotkey_set=%s",
            bool(new_cfg.mic_device), bool(new_cfg.system_audio_device),
            bool(new_cfg.output_folder), bool(new_cfg.bookmark_hotkey),
        )
