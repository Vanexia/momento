...
_ROLE_GAME = Qt.ItemDataRole.UserRole + 6   # str — humanised game name | ""

# Obsidian card metrics (design: 104x60 thumb, ~13px radius card, 8px gap).
THUMB_W = 104
THUMB_H = 60
CARD_PAD = 10
CARD_GAP = 12  # gap between thumbnail and text block
CARD_HEIGHT = THUMB_H + 2 * CARD_PAD  # 80; the delegate insets 4px → 8px row gap
    ...
    def paint(self, painter: QPainter, option: QStyleOptionViewItem, index: QModelIndex) -> None:
        ...
        try:
            ...
            if selected:
                ...
                if hovered:
                    bg = QColor("#15161d")
                    border = QColor(_theme.BORDER_HOVER)
                else:
                    bg = QColor(_theme.BG_CARD)
                    border = QColor(_theme.BORDER_SUBTLE)
                painter.setPen(QPen(border, 1))
                painter.setBrush(QBrush(bg))
            painter.drawRoundedRect(card, radius, radius)

            # ---- Selected accent bar (3px violet→magenta, inset top/bottom) ----
            if selected:
                bar = QRect(card.left(), card.top() + 14, 3, card.height() - 28)
                bar_grad = QLinearGradient(
                    float(bar.left()), float(bar.top()),
                    float(bar.left()), float(bar.bottom()),
                )
                bar_grad.setColorAt(0.0, QColor(_theme.ACCENT_VIOLET))
                bar_grad.setColorAt(1.0, QColor(_theme.ACCENT_MAGENTA))
            ...
            painter.drawRoundedRect(thumb_rect, 9, 9)
            if thumb_path:
                ...
                painter.setPen(QColor("#2c2e38"))
                painter.setBrush(Qt.BrushStyle.NoBrush)
                inner = thumb_rect.adjusted(34, 18, -34, -18)
                painter.drawRoundedRect(inner, 4, 4)
            # inset vignette so bright frames don't blow out the card edge
            vign = QColor(0, 0, 0, 70)
            painter.setPen(QPen(vign, 3))
            painter.setBrush(Qt.BrushStyle.NoBrush)
            painter.drawRoundedRect(thumb_rect.adjusted(1, 1, -1, -1), 9, 9)
            painter.restore()
            ...
            if isinstance(duration, float) and duration > 0:
                ...
                badge_font.setPointSize(max(7, option.font.pointSize() - 2))
                ...
                badge = QRect(
                    thumb_rect.right() - tw - 5,
                    thumb_rect.bottom() - th - 5,
                    tw, th,
                )
                painter.setPen(Qt.PenStyle.NoPen)
            ...
            text_w = card.right() - text_left - CARD_PAD
            raw_path = Path(index.data(_ROLE_PATH) or "")
            title = friendly_recording_title(raw_path.name) if raw_path.name else "(unnamed)"
            game = (index.data(_ROLE_GAME) or "").strip()
            mtime = index.data(_ROLE_MTIME) or 0.0
            size_bytes = index.data(_ROLE_SIZE) or 0
            # Only show the game line when it adds information (i.e. the file was
            # renamed away from its default game-named title). Otherwise a 2-line
            # title + meta block, vertically centred.
            show_game = bool(game) and game.lower() != title.lower()
            ...
            game_font = QFont(option.font)
            game_font.setPointSize(max(8, option.font.pointSize() - 1))
            game_font.setWeight(QFont.Weight.DemiBold)
            ...
            fm_game = painter.fontMetrics()

            if show_game:
                block_h = fm_title.height() + 3 + fm_game.height() + 5 + fm_meta.height()
            else:
                block_h = fm_title.height() + 5 + fm_meta.height()
            y = card.top() + (card.height() - block_h) // 2

            # Title
            painter.setFont(title_font)
            painter.setPen(QColor("#ffffff") if selected else QColor("#e6e7ee"))
            painter.drawText(
                QRect(text_left, y, text_w, fm_title.height()),
            ...
            if show_game:
                painter.setFont(game_font)
                painter.setPen(QColor("#b59ddb") if selected else QColor("#7d8088"))
                painter.drawText(
                    QRect(text_left, y, text_w, fm_game.height()),
                    Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter,
                    fm_game.elidedText(game, Qt.TextElideMode.ElideRight, text_w),
                )
                y += fm_game.height() + 5
            else:
                y += 2
            ...
            # Meta line: date · size
            painter.setFont(meta_font)
            painter.setPen(QColor("#6b6d78") if selected else QColor("#5a5c66"))
            painter.drawText(
                QRect(text_left, y, text_w, fm_meta.height()),
                Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter,
                fm_meta.elidedText(meta, Qt.TextElideMode.ElideRight, text_w),
            )
        finally:
            painter.restore()
...
class RecordingsList(QListView):
    ...
    def select_by_path(self, path: Path | str, emit: bool = True) -> bool:
        ...
        ``emit=False`` suppresses the ``currentChanged`` → ``selected_changed``
        signal, so a caller can restore a selection visually (e.g. after a
        live list rebuild) without triggering a preview reload.
        """
        target = str(path)
        for row in range(self._model.rowCount()):
            if self._model.item(row).data(_ROLE_PATH) == target:
                sm = self.selectionModel()
                was_blocked = sm.blockSignals(not emit)
                try:
    ...
    def add_item(
        ...
        item.setData(game_name or "", _ROLE_GAME)
        self._model.appendRow(item)

    def update_game(self, path: Path | str, game_name: str | None) -> None:
        row = self._row_for(path)
        if row < 0:
            return
        self._model.item(row).setData(game_name or "", _ROLE_GAME)
