...
from PyQt6.QtGui import (
    ...
    QPainterPath, QPalette, QPen, QPixmap, QPolygonF, QRadialGradient, QShortcut,
)
...
class _FullscreenOverlay(QWidget):
    ...
    def __init__(self, host: "_VideoArea", preview: "VideoPreview") -> None:
        ...
        # Single source of truth for the overlay glyph: track the embedded
        # button's mute state so a mute driven by the M shortcut or the volume
        # slider's auto-unmute (not just an overlay click) keeps the on-screen
        # fullscreen glyph in sync. Qt drops this connection when the overlay's
        # C++ object is reaped on fullscreen exit.
        self._preview._mute_btn.toggled.connect(self._sync_mute_glyph)
        self._sync_mute_glyph()  # seed for the already-muted-at-enter case
        ...
        self.on_activity()
        # Seed the play/pause glyph from the CURRENT state. The overlay is
        # rebuilt on every fullscreen-enter and its button defaults to the play
        # icon; without this, entering fullscreen while already playing leaves a
        # play icon on a playing clip (it would only update on the NEXT state
        # change, which never comes).
        self._on_playing_changed(preview.is_playing())
    ...
    def _on_mute_clicked(self) -> None:
        # toggle_mute() flips the embedded button, whose `toggled` signal drives
        # _sync_mute_glyph — so the overlay glyph updates here too, no manual set.
        self._preview.toggle_mute()
        self._return_focus_to_host()

    def _sync_mute_glyph(self, _muted: bool = False) -> None:
        """Match the overlay's mute glyph to the real audio state (single
        source of truth: the embedded playbar's mute button)."""
...
class _PlayerFrame(QWidget):
    """The rounded 'hero' player container — paints the design's dark-violet
    radial backdrop + a 14px rounded border behind the video + chrome strips."""
    ...
    def paintEvent(self, _event) -> None:  # noqa: N802
        ...
        try:
            ...
            grad = QRadialGradient(QPointF(r.center().x(), r.top()), r.width() * 1.15)
            grad.setColorAt(0.0, QColor("#2a1840"))
            grad.setColorAt(0.45, QColor("#120c1c"))
            grad.setColorAt(1.0, QColor("#07070b"))
            path = QPainterPath()
            path.addRoundedRect(r, 14, 14)
            p.fillPath(path, grad)
            p.setPen(QPen(QColor(_theme.BORDER_SUBTLE), 1))
            p.drawPath(path)
        finally:
            p.end()


def _flat_glyph_button(name: str, px: int, glyph_size: int = 20,
                       tooltip: str = "") -> QPushButton:
    """A borderless white-glyph button for the player playbar."""
    btn = QPushButton()
    btn.setCursor(Qt.CursorShape.PointingHandCursor)
    btn.setFlat(True)
    btn.setFixedSize(px, px)
    btn.setIconSize(QSize(glyph_size, glyph_size))
    btn.setIcon(icons.qicon(name, glyph_size, "#ffffff"))
    if tooltip:
        btn.setToolTip(tooltip)
    btn.setStyleSheet(
...
class _HighlightPager(QWidget):
    """Top-right glassy pill on the player: ‹ • Highlight N / M ›.

    Pages through the recording's detected highlight times (sourced from
    bookmarks). Hidden when the clip has none.
    """
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        super().__init__(parent)
        self.setObjectName("HighlightPager")
        self.setStyleSheet(
            "QWidget#HighlightPager { background: rgba(0,0,0,0.40); "
            "border: 1px solid rgba(255,255,255,0.12); border-radius: 8px; }"
            "QWidget#HighlightPager QLabel { background: transparent; }"
        )
        lay = QHBoxLayout(self)
        lay.setContentsMargins(10, 6, 10, 6)
        lay.setSpacing(9)
        self._prev = self._chevron("chevron-left")
        self._prev.clicked.connect(self.prev_clicked.emit)
        lay.addWidget(self._prev)
        dot = QLabel()
        dot.setFixedSize(7, 7)
        # Match the violet bookmark pips on the timeline (these highlights ARE
        # the bookmarks), not the yellow spark accent.
        dot.setStyleSheet(
            f"background: {_theme.ACCENT_VIOLET}; border-radius: 3px;"
        )
        lay.addWidget(dot)
        self._label = QLabel("Highlight 0 / 0")
        self._label.setStyleSheet("color: #ffffff; font-size: 12px; font-weight: 700;")
        lay.addWidget(self._label)
        self._next = self._chevron("chevron-right")
        self._next.clicked.connect(self.next_clicked.emit)
        lay.addWidget(self._next)

    def _chevron(self, name: str) -> QPushButton:
        b = QPushButton()
        b.setCursor(Qt.CursorShape.PointingHandCursor)
        b.setFlat(True)
        b.setFixedSize(15, 15)
        b.setIconSize(QSize(13, 13))
        b.setIcon(icons.qicon(name, 13, "#ffffff"))
        b.setStyleSheet(
            "QPushButton { background: transparent; border: none; "
            "padding: 0; margin: 0; min-width: 0; min-height: 0; }"
            "QPushButton:hover { background: rgba(255,255,255,0.12); border-radius: 5px; }"
        )
        return b

    def set_index(self, index: int, total: int) -> None:
        self._label.setText(f"Highlight {index} / {total}")
...
class VideoPreview(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        # Fullscreen expands the EDITOR window in place (the embedded video
        # widget is never reparented) so the video keeps rendering into the
        # same window handle that screen-share / window-capture follows. The
        # window + surrounding-chrome work is delegated to a callback the
        # editor installs via set_fullscreen_window_callback().
        self._is_fullscreen = False
        self._fullscreen_window_cb: Callable[[bool], None] | None = None
        ...
        # Video letterboxes on black; give the native video sub-window a black
        # background brush so its area never reads white. (The blinding
        # window-open flash itself is handled at the window level in
        # EditorWindow.showEvent via an opacity reveal.)
        ...
        self._video_empty_label.setStyleSheet(
            "QLabel { background-color: transparent; color: #8a92a3; font-size: 11pt; }"
        )
        self._video_stack = QStackedWidget()
        self._video_stack.setStyleSheet("background: transparent;")
        ...
        # --- player header: clip title + subline (left), highlight pager (right) ---
        ...
        # Centre the title (+ optional subline) vertically within the header band
        # so it isn't top-biased once the subline collapses.
        title_col.addStretch(1)
        title_col.addWidget(self._title_label)
        title_col.addWidget(self._subline_label)
        title_col.addStretch(1)

        self._pager = _HighlightPager()
        self._pager.prev_clicked.connect(self._goto_prev_highlight)
        self._pager.next_clicked.connect(self._goto_next_highlight)
        self._pager.setVisible(False)
        self._highlights: list[float] = []

        self._player_header = QWidget()
        self._player_header.setStyleSheet("background: transparent;")
        header = QHBoxLayout(self._player_header)
        header.setContentsMargins(18, 10, 18, 4)
        header.setSpacing(12)
        header.addLayout(title_col, stretch=1)
        header.addWidget(self._pager, alignment=Qt.AlignmentFlag.AlignVCenter)

        # --- playbar controls (uniform 32px height so glyphs share one baseline) ---
        self._play_btn = _flat_glyph_button("play", 32, 20, "Play")
        self._play_btn.clicked.connect(self.toggle_play)

        self._time_label = QLabel("0:00 / 0:00")
        self._time_label.setMinimumWidth(110)
        self._time_label.setStyleSheet(
            "color: #ffffff; font-size: 12.5px; font-weight: 600; background: transparent;"
        )
        self._time_label.setAlignment(Qt.AlignmentFlag.AlignLeft | Qt.AlignmentFlag.AlignVCenter)

        self._mute_btn = _flat_glyph_button("volume", 32, 18, "Mute")
        ...
        self._volume_slider.setFixedWidth(76)
        self._volume_slider.setFixedHeight(22)
        self._volume_slider.setStyleSheet(_PLAYBAR_SLIDER_QSS)
        self._volume_slider.setToolTip("Preview volume (this affects playback only, not the file)")
        self._volume_slider.valueChanged.connect(self._on_volume_changed)

        self._fullscreen_btn = _flat_glyph_button("fullscreen", 32, 17, "Fullscreen")
        self._fullscreen_btn.clicked.connect(self.toggle_fullscreen)

        # Controls live in their own widget so fullscreen can hide them as a
        # unit (the floating overlay takes over there).
        self._controls_bar = QWidget()
        self._controls_bar.setStyleSheet("background: transparent;")
        controls = QHBoxLayout(self._controls_bar)
        controls.setContentsMargins(18, 10, 18, 10)
        controls.setSpacing(12)
        # Center every item on a single horizontal baseline — without an explicit
        # alignment, smaller fixed-size buttons get top-aligned in the row and
        # ride a couple of pixels high next to the play button / time text.
        _vc = Qt.AlignmentFlag.AlignVCenter
        controls.addWidget(self._play_btn, 0, _vc)
        controls.addWidget(self._time_label, 0, _vc)
        controls.addStretch(1)
        controls.addWidget(self._mute_btn, 0, _vc)
        controls.addWidget(self._volume_slider, 0, _vc)
        controls.addWidget(self._fullscreen_btn, 0, _vc)

        # --- player frame (radial bg + rounded border) holds header/video/playbar ---
        self._player_frame = _PlayerFrame()
        frame_lay = QVBoxLayout(self._player_frame)
        frame_lay.setContentsMargins(1, 1, 1, 1)
        frame_lay.setSpacing(0)
        frame_lay.addWidget(self._player_header)
        frame_lay.addWidget(self._video_stack, stretch=1)
        frame_lay.addWidget(self._controls_bar)

        # --- layout ---
        root = QVBoxLayout(self)
        root.setContentsMargins(18, 18, 18, 0)
        root.setSpacing(0)
        root.addWidget(self._player_frame, stretch=1)
    ...
    def load(self, path: Path | str | None) -> None:
        ...
        # Invalidate any first-frame timer scheduled for the previous source.
    ...
    def current_path(self) -> Path | None:
        """The recording currently loaded into the player, or None if cleared.

        The editor uses this to load/unload the (heavy) video source as the
        window is shown/hidden without double-loading the same clip."""
        return self._current_path

    def set_clip_meta(self, title: str | None, subline: str | None) -> None:
        """Set the player's top-left overlay text (clip title + game/format)."""
        self._title_label.setText(title or "")
        self._subline_label.setText(subline or "")
        # Collapse the subline row when empty so the header is a single slim line
        # (common case: a recording whose title already is the game name).
        self._subline_label.setVisible(bool(subline))

    def set_highlights(self, times) -> None:
        """Feed the highlight pager its marker times (seconds, sourced from
        bookmarks). Hides the pager when the clip has none."""
        self._highlights = sorted(
            float(t) for t in (times or []) if t is not None and float(t) >= 0
        )
        has = len(self._highlights) > 0
        self._pager.setVisible(has)
        self._update_pager_index()

    def _current_highlight_index(self) -> int:
        """0-based index of the highlight the playhead is currently on (the
        last one at/before the current position), or -1 if before the first."""
        pos = self.position()
        cur = -1
        for i, t in enumerate(self._highlights):
            if t <= pos + 0.25:
                cur = i
        return cur

    def _update_pager_index(self) -> None:
        total = len(self._highlights)
        if total == 0:
            return
        self._pager.set_index(max(1, self._current_highlight_index() + 1), total)

    def _goto_prev_highlight(self) -> None:
        if not self._highlights:
            return
        # Step to the PREVIOUS highlight by index — one click from "2 / 2" goes
        # straight to highlight 1, instead of restarting the current one (the
        # old position-relative logic jumped to the marker just before the
        # playhead, which after playing a few seconds is the current highlight).
        target = max(0, self._current_highlight_index() - 1)
        self.seek(self._highlights[target])
        QTimer.singleShot(0, self._update_pager_index)

    def _goto_next_highlight(self) -> None:
        if not self._highlights:
            return
        target = min(len(self._highlights) - 1, self._current_highlight_index() + 1)
        self.seek(self._highlights[target])
        QTimer.singleShot(0, self._update_pager_index)
    ...
    def _set_controls_enabled(self, enabled: bool) -> None:
        ...
        """
        for w in (
            self._play_btn, self._mute_btn, self._volume_slider,
    ...
    def set_fullscreen_window_callback(self, cb: Callable[[bool], None] | None) -> None:
        """Install the editor's hook that makes the EDITOR WINDOW fullscreen
        (and hides/restores its surrounding panels). Called with True to enter,
        False to exit. Keeping fullscreen inside the editor window — rather
        than a separate top-level — means the video keeps rendering into the
        window handle Discord/OBS window-capture (and screen-share) follow."""
        self._fullscreen_window_cb = cb
    ...
    def _enter_fullscreen(self) -> None:
        if self._is_fullscreen or self._fullscreen_window_cb is None:
            return
        self._is_fullscreen = True
        # Fill the screen edge-to-edge for aspect-matched recordings (the
        # universal case here); slight edge-crop on mismatched footage beats
        # letterbox bars for gameplay where the HUD sits centred.
        self._video_widget.setAspectRatioMode(
            Qt.AspectRatioMode.KeepAspectRatioByExpanding
        )
        self._controls_bar.setVisible(False)
        self._player_header.setVisible(False)
        # Editor window goes fullscreen + hides its chrome. The embedded video
        # widget is NOT reparented and keeps the player's output, so capture
        # follows it.
        self._fullscreen_window_cb(True)
        # Floating YouTube-style overlay anchored over the now-fullscreen
        # embedded video. Built after the window is fullscreen so it picks up
        # the final geometry.
        overlay = _FullscreenOverlay(self._video_widget, self)
        overlay.show()
        overlay.raise_()
    ...
    def _exit_fullscreen(self) -> None:
        ...
        # The overlay hides the cursor (BlankCursor on the video widget) after
        # a few seconds of inactivity in fullscreen. If we exited while it was
        # hidden, that BlankCursor would otherwise stick to the embedded player
        # — making the mouse invisible over the video in windowed mode. Restore
        # it on every exit.
        self._video_widget.unsetCursor()
        self._controls_bar.setVisible(True)
        self._player_header.setVisible(True)
        # Back to letterbox-fit for the small embedded preview.
        self._video_widget.setAspectRatioMode(Qt.AspectRatioMode.KeepAspectRatio)
        # Editor window leaves fullscreen + restores its chrome.
        if self._fullscreen_window_cb is not None:
            self._fullscreen_window_cb(False)
    ...
    def _on_player_duration(self, ms: int) -> None:
        ...
        if self._prime_pending and ms > 0:
            ...
            if self._player.playbackState() != QMediaPlayer.PlaybackState.PlayingState:
                ...
                QTimer.singleShot(80, lambda: self._prime_pause(generation))

    def _prime_pause(self, generation: int) -> None:
    ...
    def _set_mute_icon(self, muted: bool) -> None:
        self._mute_btn.setIcon(icons.qicon("mute" if muted else "volume", 18, "#ffffff"))
        self._mute_btn.setToolTip("Unmute" if muted else "Mute")

    # ------------------------------------------------------------ helpers
    def _set_play_icon(self, playing: bool) -> None:
        self._play_btn.setIcon(icons.qicon("pause" if playing else "play", 20, "#ffffff"))
