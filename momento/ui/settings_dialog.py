...
from momento.core.storage_cleanup import MigrationWorker, count_movable
...
class _MigrationDriver(QObject):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # An unexpected (non-OSError) failure must NOT leave the modal,
            # cancel-button-less progress dialog open forever (which forces the
            # user to kill the app). Always emit finished so it closes.
            logger.exception("Folder migration worker crashed")
            failed = max(failed, 1)
        finally:
            # Keep the result independently of queued GUI delivery. Application
            # shutdown can end the dialog's nested event loop before the signal
            # reaches the main thread, but the caller still drains this worker.
            self._result["moved"] = moved
            self._result["failed"] = failed
            self.finished.emit(moved, failed)


class _MigrationProgressDialog(QProgressDialog):
    """Progress dialog that cannot be dismissed while files are moving."""
...
# Settings nav rail — (page_key, display_label, glyph). page_key matches the
# old tab text so open_tab()'s existing callers ("Audio", "Output") still work;
# "Bookmarks" is displayed as "Highlights" per the redesign.
_ALL_NAV_ITEMS = (
    ("Audio", "Audio", "speaker"),
    ("Capture", "Capture", "capture"),
    ("Output", "Output", "output"),
    ("Bookmarks", "Highlights", "star"),
    ("Games", "Games", "games"),
...
class _NavItem(QFrame):
    """One row in the settings nav rail: glyph + label, with an active state
    (violet gradient + inset ring)."""

    clicked = pyqtSignal(int)

    def __init__(self, index: int, label: str, glyph: str, parent=None) -> None:
    ...
    def set_active(self, active: bool) -> None:
        ...
        if active:
            self.setStyleSheet(
                "_NavItem { border-radius: 10px; background: qlineargradient("
                "x1:0, y1:0, x2:1, y2:1, stop:0 rgba(124,58,237,0.18), "
            ...
            self.setStyleSheet(
                "_NavItem { border-radius: 10px; background: transparent; border: 1px solid transparent; }"
...
class SettingsPanel(QWidget):
    ...
    def __init__(
        ...
        *,
        ...
        # ---- Header row: Back (secondary) + "Settings" title ----
        header = QFrame()
        header.setObjectName("SettingsHeader")
        header.setFixedHeight(60)
        header.setStyleSheet(
            f"QFrame#SettingsHeader {{ background: {_theme.BG_CHROME}; "
            f"border-bottom: 1px solid {_theme.BORDER_HAIRLINE}; }}"
            "QFrame#SettingsHeader QLabel { background: transparent; }"
        )
        ...
        self._back_btn.setIcon(icons.qicon("chevron-left", 15, _theme.ACCENT_VIOLET))
        self._back_btn.setIconSize(QSize(15, 15))
        ...
        # ---- Body: nav rail (left) + content stack with global footer ----
        body = QHBoxLayout()
        ...
        builders = {
            ...
            "Output": self._build_output_tab,
            "Bookmarks": self._build_bookmarks_tab,
            "Games": self._build_games_tab,
            "Notifications": self._build_notifications_tab,
            "Startup": self._build_startup_tab,
            "YouTube": self._build_youtube_tab,
        }
        for key, _label, _glyph in _NAV_ITEMS:
            self._page_index[key] = self._stack.addWidget(builders[key]())
        ...
        content_wrap.setStyleSheet(
            f"QWidget#SettingsContent {{ background-color: {_theme.BG_APP}; }}"
        )
        cw = QVBoxLayout(content_wrap)
        cw.setContentsMargins(0, 0, 0, 0)
        cw.setSpacing(0)
        # Save/Cancel dock under each tab's content card (see _tab_with), so the
        # content area itself is just the scrolling stack.
        cw.addWidget(self._stack, stretch=1)
    ...
    def _build_nav_rail(self) -> QWidget:
        rail = QFrame()
        rail.setObjectName("SettingsNav")
        rail.setFixedWidth(232)
        rail.setStyleSheet(
            f"QFrame#SettingsNav {{ background: {_theme.BG_RAIL}; "
        ...
        eyebrow.setStyleSheet(
            f"color: {_theme.TEXT_FAINT}; font-size: 10.5px; font-weight: 700; "
            "letter-spacing: 1.4px; padding: 4px 12px 8px;"
        )
        lay.addWidget(eyebrow)
        self._nav_items: list[_NavItem] = []
        for i, (_key, label, glyph) in enumerate(_NAV_ITEMS):
            item = _NavItem(i, label, glyph)
    ...
    def open_tab(self, name: str) -> None:
        """Switch to a named page (case-insensitive). Matches the page key
        ("Bookmarks") or its display label ("Highlights"). No-op if unknown."""
        want = name.lower()
        for i, (key, label, _glyph) in enumerate(_NAV_ITEMS):
            if want in (key.lower(), label.lower()):
                self._select_nav(i)
                return
    ...
    def _build_capture_tab(self) -> QWidget:
        return _tab_with_groups(
            ...
            _tips_group(
                "Quality guide",
                [
                    "High gives the best image but can use more than 10 GB per "
                    "hour at 1440p60. Medium or Custom uses less space.",
                ],
            ),
            _tips_group(
                "Capture tips",
    ...
    def _on_sys_audio_test_clicked(self) -> None:
        ...
        target = match_audio_output(device_id)
    ...
    def _build_capture_group(self) -> QGroupBox:
        ...
        self._quality_combo.setToolTip(
            "Low, Medium, and High target consistent visual quality, so file "
            "size varies. Custom uses the bitrate below."
        )
        ...
        self._bitrate_spin.setToolTip(
            "Used only with Custom bitrate. Try 8–12 Mbit/s for 1080p60, "
            "16–20 Mbit/s for 1440p60, or 25–40 Mbit/s for 4K60."
        )
        self._bitrate_row_label = QLabel("Custom bitrate (inactive):")
        layout.addRow(self._bitrate_row_label, self._bitrate_spin)
        self._bitrate_spin.setEnabled(False)
    ...
    def _on_quality_changed(self, _index: int) -> None:
        """Keep bitrate state and quality guidance in sync with the preset."""
        data = self._quality_combo.currentData()
        is_custom = data == "custom"
        self._bitrate_row_label.setText(
            "Custom bitrate:" if is_custom else "Custom bitrate (inactive):"
        )
        self._bitrate_spin.setEnabled(is_custom)
    ...
    def _on_yt_connect_clicked(self) -> None:
        ...
        # This is intentionally a daemon Python thread, not a QThread parented
        # to SettingsPanel. OAuth can block for five minutes; destroying a
        # parented QThread while it is running deadlocks/aborts Qt. The worker's
        # Qt signals still queue safely back to the GUI thread, and the daemon
        # cannot hold process shutdown hostage if the user quits mid-sign-in.
        self._yt_thread = threading.Thread(
            target=self._yt_worker.run,
            name="YouTubeOAuth",
            daemon=True,
        )
    ...
    def _add_game_if_new(self, exe: str) -> bool:
        ...
        if not exe.lower().endswith(".exe"):
            exe = exe + ".exe"
        # Launchers/lobbies Momento deliberately refuses to record: from_dict
        # strips these from every loaded config, so accepting one here would
        # "work" for one session and then silently vanish on restart — tell
        # the user up front instead.
        if exe.lower() in Config._AUTO_PRUNE_GAMES:
            QMessageBox.information(
                self, "Momento",
                f"{exe} is a game launcher or lobby process, not a game.\n\n"
                "It lingers in the background and would trigger recordings of "
                "the launcher window, so Momento deliberately ignores it. Add "
                "the game's own executable instead.",
            )
            return False
        # Already present?
        for row in range(self._games_table.rowCount()):
    ...
    def _maybe_migrate_recordings(self, new_folder: Path) -> bool | None:
        """If the output folder changed and the old location has recordings
        or clips, ask the user whether to move them.

        Returns True for Move, False when no move is needed / the user chooses
        Leave, and None for Cancel. This method deliberately only decides; the
        transfer starts after the new config has been persisted successfully.
        """
    ...
    def _run_migration_with_progress(
        ...
        progress = _MigrationProgressDialog("Preparing…", "", 0, total, self)
        ...
        # Keep ownership local. If application shutdown ends the nested modal
        # loop, a parented running QThread could otherwise be destroyed along
        # with this panel and abort the process.
        thread = QThread()
        result = {"moved": 0, "failed": 0}
        driver = _MigrationDriver(worker, pairs, result)
        ...
        driver.progress_changed.connect(
            _on_progress, type=Qt.ConnectionType.QueuedConnection
        )
        driver.finished.connect(_on_finished, type=Qt.ConnectionType.QueuedConnection)
        thread.started.connect(driver.run)
        driver.finished.connect(driver.deleteLater)
        # quit() is thread-safe. A direct connection ensures the worker can end
        # its event loop even when the GUI event loop is already shutting down.
        ...
        # QApplication.quit() or an unexpected programmatic rejection can end
        # exec() before _on_finished is delivered. Never let this local thread
        # escape the method while running; that is a fatal Qt lifecycle error.
        if thread.isRunning():
            thread.quit()
            thread.wait()
        progress.finish()
    ...
    def _on_save(self) -> None:
        ...
        if not output_text:
            QMessageBox.warning(self, "Momento", "Choose an output folder before saving.")
            return
        try:
            output_folder = Path(output_text).expanduser().resolve()
        except (OSError, RuntimeError) as e:
            QMessageBox.warning(self, "Momento", f"Invalid output folder:\n{e}")
            return
        ...
        try:
            folder_changed = (
                Path(self._config.output_folder).expanduser().resolve() != output_folder
            )
        ...
        new_cfg = Config(
            ...
            quality_preset=self._quality_combo.currentData() or "custom",
        ...
        # Decide whether existing media should move, but do not touch it yet.
        # Validation above and persistence below are the transaction boundary:
        # moving first could leave the app pointing at the old, now-empty folder
        # if validation or the config write then failed.
        migrate = self._maybe_migrate_recordings(output_folder)
        if migrate is None:
            return
...
_QUALITY_DESCRIPTIONS = {
    ...
    "high": (
        "Best visual quality. At 1440p60, High can exceed 10 GB per hour "
        "because its bitrate is not capped. The Custom bitrate below is inactive."
    ),
    "custom": (
        "Set a fixed bitrate for predictable file sizes. "
        "Try 16–20 Mbit/s for 1440p60."
    ),
}
...
def _tab_with(
    ...
    *,
    ...
    # Cancel/Save dock directly under the tab's content card (bottom-right of
    # the panel) rather than at the window's bottom edge.
    if stretch_last:
