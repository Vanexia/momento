...
class TrimWorker(QObject):
    ...
    def __init__(
        ...
        self._temp_output = output_path.with_name(f".{output_path.name}.partial")
    ...
    def _run_inner(self) -> None:
        ...
        if rc != 0:
            ...
            return
        if not self._temp_output.exists():
            self.failed.emit(f"Output file missing — see {log_path}")
            return
        size = self._temp_output.stat().st_size
        ...
        try:
            os.replace(self._temp_output, self._output)
        except OSError as e:
            self.failed.emit(f"Could not publish exported clip: {e}")
            return
