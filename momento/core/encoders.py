...
The generic diagnostic probe is cached per process. Recording selection probes
the requested dimensions, frame rate, and quality profile each time because a
driver can accept a small default context and reject the real session.
...
def _probe_one(
    ...
    *,
    ...
    Probing with default options used to mask bugs where the encoder opened bare
    but rejected the requested quality options at record time. The caller's
    profile is applied here, then Recorder opens and primes the full output
    stream before publishing the session.
    ...
    try:
        ...
        ctx.time_base = Fraction(1, int(framerate))
        ctx.framerate = Fraction(int(framerate))
        ctx.gop_size = max(1, int(framerate) * 2)
        ...
        ctx.options = quality_options_for(name, preset, custom_bitrate_kbps)
        ctx.open()
...
def detect_available() -> list[str]:
    ...
    return [r.encoder for r in _probe_cache if r.ok and r.encoder not in _runtime_disabled]


def disable_for_process(encoder: str, error: BaseException | str) -> None:
    """Skip a failed backend until Momento restarts and probes afresh."""
    if encoder not in _PRIORITY:
        return
    detail = str(error).strip() or type(error).__name__
    if encoder not in _runtime_disabled:
        logger.warning(
            "Disabling video encoder for this Momento run: %s (%s)",
            encoder,
            detail,
        )
    _runtime_disabled[encoder] = detail
...
def pick_encoder_for_recording(
    *,
    width: int,
    height: int,
    framerate: int,
    ...
    excluded: tuple[str, ...] | set[str] = (),
) -> str:
    """Pick a backend that opens with this recording's production settings.

    A low-resolution generic probe can pass while the same driver rejects a
    1440p/4K session, a 60+ fps rate, or a different rate-control mode. Probe
    every backend in priority order with the exact target dimensions, frame
    rate, and quality profile so vendor-specific failures reach the next
    hardware backend or the software floor before recording begins.
    """
    results: list[_ProbeResult] = []
    skipped = set(excluded) | set(_runtime_disabled)
    for name in _PRIORITY:
        ...
        result = _probe_one(
            ...
            framerate=framerate,
            preset=preset,
            custom_bitrate_kbps=custom_bitrate_kbps,
        )
        results.append(result)
        logger.info(
            "Recording encoder probe: %s %dx%d@%d preset=%s = %s%s",
            result.encoder,
            width,
            height,
            framerate,
            preset,
            "available" if result.ok else "unavailable",
    ...
    details = "; ".join(
        f"{result.encoder}: {result.error or 'unavailable'}" for result in results
    )
    if skipped:
        details = f"skipped={','.join(sorted(skipped))}; {details}"
    raise RuntimeError(
        "No H.264 encoder could open for "
        f"{width}x{height}@{framerate} preset={preset}. {details}"
    )
