...
class BookmarkStore:
    ...
    def add(self, seconds: float) -> bool:
        ...
        with self._lock:
            ...
            self._items.sort()
            # Keep persistence inside the same lock as the snapshot mutation.
            # Otherwise an older, slower write can finish after a newer one and
            # silently remove the most recent bookmark from disk.
            try:
