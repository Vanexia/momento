...
def protect(plaintext: bytes, entropy: bytes | None = None) -> bytes:
    ...
    ``entropy`` is optional application-specific secondary entropy: when given,
    the same bytes must be supplied to :func:`unprotect`. It binds the blob to
    the app as well as the user, so generic same-user secret-stealers can't
    decrypt it without also knowing the entropy.
    """
...
def unprotect(ciphertext: bytes, entropy: bytes | None = None) -> bytes:
    ...
    ``entropy`` must match what was passed to :func:`protect`. Raises
    ``DPAPIError`` on tampering, wrong user account, wrong machine, or wrong
    entropy. Callers should treat the failure as "discard this token blob, user
    needs to re-auth" rather than retrying.
    """
    if _CRYPT32 is None or _KERNEL32 is None:
        raise DPAPIError("DPAPI not available on this platform")
    if not ciphertext:
