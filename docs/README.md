...
The repository's `.github/workflows/pages.yml` workflow publishes this folder.
It runs when files under `docs/` change and can also be started manually from
the Actions page.

In **Settings → Pages**, set the source to **GitHub Actions**. Push the changed
files to `main`, then check the `Deploy docs to Pages` workflow.
...
Both pages are single-file static HTML. There is no Jekyll build or local
