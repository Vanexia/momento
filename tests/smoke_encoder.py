...
def main() -> int:
    ...
    # Verify packet timing before decoding. DTS must increase strictly; equal
    # values are accepted by Matroska but trigger compatibility warnings and
    # can produce uneven playback in stricter consumers.
    print(f"\nVerifying {OUT_PATH}")
    inp = av.open(str(OUT_PATH))
    try:
        vs = inp.streams.video[0]
        video_dts = [packet.dts for packet in inp.demux(vs) if packet.dts is not None]
        dts_is_strict = all(current > previous for previous, current in zip(video_dts, video_dts[1:]))
        print(f"  strictly increasing video DTS: {dts_is_strict} ({len(video_dts)} packets)")
    finally:
        inp.close()

    # Verify by fully decoding the MKV.
    inp = av.open(str(OUT_PATH))
    try:
        vs = inp.streams.video[0]
    ...
    if a_seconds < DURATION_S * 0.9:
        ...
        return 4
    if not dts_is_strict:
        print("FAIL: video DTS is not strictly increasing", file=sys.stderr)
        return 5
    print("\nPASS")
    return 0
