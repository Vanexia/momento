"""Console-only SessionManager runner for manual recording checks.
...
Usage (PowerShell):
    ...
    C:\\dev\\Momento\\.venv\\Scripts\\python.exe tests\\test_session.py `
        --mic "USB Microphone" `
        --audio "Speakers (USB Audio Device)" `
...
def main() -> int:
    ...
    parser.add_argument("--mic", required=True, help="Microphone endpoint name")
    parser.add_argument("--audio", required=True, help="System audio endpoint name")
    parser.add_argument("--output", required=True, help="Folder to write MKV recordings into")
